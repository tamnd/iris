...
### A table provider has to ask before it can push

`iris-df` registers a container as a DataFusion table. A query on it becomes one leaf node holding a list of tuple ranges, one per output partition, and each partition opens the container, reads its own rows and closes it. Splitting a scan that way is safe because of what M5 already checks rather than because it looks safe: a range read twice comes back byte identical and ranges read out of order concatenate to the sequential result, against every decoder in the tree.

The part that was not obvious is that a table provider cannot tell from a container whether it may push a projection into it. The metadata says what the decoder requires of the host, which is the right thing to record and is not the same question. What the decoder offers only exists after a handshake, and the runtime refuses a scan that names columns to a decoder that never agreed to projection, so a provider that guessed would either refuse queries it could serve or send requests that come back as errors. So `Dataset::capabilities` and `Windowed::capabilities` were added: one instantiation and one handshake, no rows read, asked once when a table is opened. A decoder that agreed is told which columns to read and a decoder that did not has every column read and the wanted ones taken out of the batches afterwards. Both give the same answer and only one of them moves fewer bytes, which is why `IrisTable::pushes_projection` is a question a host can ask before it plans anything.

The gate is the request count and not the rows, because the rows cannot tell the two apart. `IrisTable::traffic` sums what every partition asked of the source, and the test that matters reads three columns of a seven megabyte file and then one of them: the projected scan moves a fraction of what the whole scan moves, through a decoder that decided which bytes to fetch. Filters are reported as unsupported rather than inexact. The ABI has a field for one and a capability bit for it, and there is no agreed encoding for what goes in the field and no decoder that implements the bit, so claiming a filter is partly handled would be a claim about work that does not happen.

What it costs today is one module compilation per partition, because opening a container compiles the decoder in it and each partition opens its own. That is written into the crate docs rather than left to be discovered, and it is what the compiled module cache is for. The other cost is that each partition reads inside `spawn_blocking`, because a range miss blocks the thread serving it, which is the last box in this milestone.
...
So it lives in `iris-vm`, which moved from `forbid(unsafe_code)` to `deny` with the two places that need it allowing it. That is a real weakening and the crate's own documentation says which claim it weakens: not the one about `unsafe impl Send`, which `ci/discipline.py` enforces and which is unchanged, but the blanket statement that no unsafe call can appear there. What replaced it is that one Wasmtime function is called unsafely, from one private wrapper that says what a caller is promising, and none of it is reachable from outside the crate, so there is no unsafe function in the public API. Every other module still cannot use it without adding an attribute somebody will see in review.
