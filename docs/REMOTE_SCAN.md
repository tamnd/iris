...
**On wall clock it is behind on every shape.** The projected scan is 125.4 ms coalesced against 25.9 ms for parquet-tuned. About 34 of those milliseconds are the open, and only about three of those 34 are the network, by the round trip figure and the five requests in the table. The other 31 are local work: parsing the footer, hashing the decoder section and compiling it to machine code. There was no compiled module cache in `iris-runtime` when this was measured, so every `open_windowed` paid that again, and the hash that would key such a cache was already being computed for verification. That is a fixed cost per open and it was the single largest addressable item in this table. There is one now, off unless an operator names a directory, and `docs/COLD_START.md` measures what it is worth. The numbers in this table were taken before it existed and have not been re-run.
