...
The dry run is one `cargo package --workspace` rather than thirteen `cargo publish --dry-run` calls, and the difference matters on a first release. A single crate dry run resolves that crate's dependencies against crates.io, so on a first release everything downstream of `iris-abi` fails to resolve for a reason that has nothing to do with whether the release is good. Packaging the workspace builds a temporary registry out of the workspace itself, so each crate is verified against the versions that are about to go out.

`--workspace` there means every member and not every publishable member, so the crates marked `publish = false` are packaged too, and one of them cannot be. `iris-duckdb` depends on `iris-c` by path with no version on it, which is right, because `iris-c` produces a shared library and a header rather than a crate anybody installs and is never going on crates.io for a version to point at. Packaging refuses a dependency with no version, so a crate that is not part of the release failed the rehearsal for the twelve that are, which is how the `0.9.0` dry run first came back red. The script now excludes every member whose manifest says `publish = false`, and it reads that list out of `cargo metadata` rather than keeping a copy, because a copy is wrong the first time somebody adds a crate.
