...
| `iris-parquet` | A container carried inside a Parquet file, in the gap unmodified readers never look at. |
...
The DataFusion integration is `iris-df`. The C ABI is `iris-c`, described for a C programmer in [`docs/C_ABI.md`](docs/C_ABI.md), and it ships as a shared library, a static library and a header attached to a release rather than as a crate. The Python bindings are `iris-py`, described in [`docs/PYTHON.md`](docs/PYTHON.md), and they ship as a wheel called `irisdb` that you import as `iris`. The DuckDB extension is `iris-duckdb`, described in [`docs/DUCKDB.md`](docs/DUCKDB.md), and it ships as a `.duckdb_extension` file attached to a release. The Parquet embedding is `iris-parquet`, described in [`docs/PARQUET.md`](docs/PARQUET.md): a container carried inside a Parquet file, in the gap between the last row group and the footer, where a reader that has never heard of iris never looks. The argument for doing it that way, with what it costs a reader that has never heard of iris measured against a reader that has never heard of iris, is [`docs/PARQUET_PROPOSAL.md`](docs/PARQUET_PROPOSAL.md).
