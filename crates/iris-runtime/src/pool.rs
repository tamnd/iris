...
mod tests {
    ...
    fn a_build_that_fails_is_not_remembered() {
        ...
        assert_eq!(
            pool.get_or_build(&1, 8, || Err::<String, _>("no")),
            Err("no")
        );
...
mod loom_model {
    ...
    ///
    /// One thread can only fail and the other can only succeed, and which of them claims the key is
    /// the model's to choose. The claim being checked is about the thread that can succeed: it comes
    /// back with a value in every ordering, whether it built one itself, found one already held, or
    /// was parked waiting on a build that then produced nothing. The last of those is the one worth
    /// enumerating, because a wakeup after a build that has nothing to hand over is the one a
    /// failure path has no obvious reason to send and the one whose absence is a hang.
    ///
    /// Nothing is asserted about the thread that fails beyond its answer being one of the two an
    /// honest pool could give. If it claimed first it gets its own error back, and if it arrived
    /// second it is handed the value the other thread built and never runs its closure at all. Both
    /// are correct, and a model that insisted on the first of them would be asserting a scheduling
    ...
    fn loom_a_failed_build_hands_the_key_to_whoever_is_waiting() {
        loom::model(|| {
            ...
            let failed = failing.join().expect("no model here panics");
            let succeeded = succeeding.join().expect("no model here panics");
            assert_eq!(
                succeeded,
                Ok(70),
            ...
            assert!(
                failed == Err(1) || failed == Ok(70),
                "a thread that cannot build this key either says so or is handed what the other one \
                 built, and it came back with {failed:?}"
            );
        });
    }
